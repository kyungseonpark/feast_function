...
async def __write_file_contents(file_path: str, contents: str):
    """
...
async def __write_yaml_contents(file_path: str, contents: dict):
    """
    Writes the content to YAML in the specified path.

        :param file_path: The path to the file to write.
        :param contents: The contents of the file, which should be written in the form of a Dictionary.
    """
    async with aiofiles.open(file_path, "w", encoding="utf-8") as f:
        await f.write(yaml.dump(contents))
    

async def make_feast_init_files(workspace_id: int):
    """
    Create the files generated by 'feast init'. __init__.py and feature_store.yaml.

        :param workspace_id: ID of the workspace used to ABACUS Studio.
        :return: name of workspace(Feature Store)
    """
    write_init_files_tasks = list()
    ws_name = get_ws_name(workspace_id)

    # Directly generate __init__.py, which is generated via 'feast init'.
    ws_init_file = get_ws_feast_init_file(workspace_id)
    write_init_files_tasks.append(__write_file_contents(ws_init_file, ''))

    # Create a configuration file for FEAST, feature_store.yaml.
...
def __is_parquet(filename: str):
    """
    Verify that the file has the extension 'parquet'.
...
async def save_parquet_file(workspace_id: int, project_id: int, dataset_id: int, parquet_file: UploadFile):
    ...
    # Save the dataset to the Feature Store.
...
async def __combine_files(dir_path: str, file_path: str, contents: str):
...
async def perform_apply(workspace_id: int):
    ...
    # Asynchronous operations to simultaneously generate configuration files within a workspace(Feature Store).
    await asyncio.gather(*combine_files_tasks)
    # Perform 'feast apply'
    # The -c option support it to perform in the feature store directory of the workspace(Feature Store).
...
async def __write_base_fv_file(workspace_id: int, project_id: int, dataset_features: dict):
    base_fv = get_base_fv_path(workspace_id, project_id)
...
async def write_base_file(
    ...
    await asyncio.gather(*write_base_file_tasks)


async def push_data(workspace_id: int, project_id: int, input_data: dict[str, list]):
    feast_path = get_ws_feast_path(workspace_id)
    fs = FeatureStore(feast_path)
    pj_name = get_pj_name(project_id)
    input_df = pandas.DataFrame.from_dict(input_data)
    fs.write_to_offline_store(f'fv_{pj_name}', df=input_df)
    return "success"

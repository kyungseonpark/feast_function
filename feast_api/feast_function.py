...
class FeastRepoItem(BaseModel):
    """Request Body in FastAPI to set the Feast repository."""
    workspace_id: int
    project_id: int


class FeastDatasetItem(FeastRepoItem):
    """Request Body for FastAPI to modify the data set stored in Feast."""
    dataset_id: int
    dataset_features: Optional[dict] = None
    entity_name: Optional[str] = None
    entity_dtype: Optional[str] = None
    timestamp_column: Optional[str] = None


class FeastServeItem(FeastRepoItem):
...
class FeastRepo:
    ...
    def __init__(self, item: FeastRepoItem):
        self._ws_name = f'workspace_{item.workspace_id}'
        self._pj_name = f'project_{item.project_id}'
        self._ws_feast_path = f'{FEAST_REPO}/{self._ws_name}'
        # workspace 당 1개씩 생성.
        self._repo_path = {
            PARQUET_FILE: f'{self._ws_feast_path}/parquet',
            FEATURE_VIEW: f'{self._ws_feast_path}/fv',
            FILE_SOURCE: f'{self._ws_feast_path}/sources',
            ENTITY: f'{self._ws_feast_path}/entities'
        }

    def feast_init(self):
        os.makedirs(self._ws_feast_path, exist_ok=True)
        for _, path in self._repo_path.items():
            os.makedirs(path, exist_ok=True)
        with open(self._ws_feast_path + "/__init__.py", "w", encoding="utf-8"):
            pass
        with open(self._ws_feast_path + "/feature_store.yaml", "w", encoding="utf-8") as f:
    ...
    def __feast_yaml(self):
        ...
        return define_feast_yaml(
            project=self._ws_name,
            registry=f'{self._ws_name}.db',
            provider='local',
...
class FeastDataset(FeastRepo):
    ...
    def __init__(self, item: FeastDatasetItem):
        super().__init__(item)
        self._dataset_id = item.dataset_id
        self._ds_name = f'dataset_{item.dataset_id}'
